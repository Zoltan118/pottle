...
const { NEXT_PUBLIC_POTTLE_ADDRESS: POTTLE } = env("../.env.local");
...
for (const [to, amt] of [[ana.address, 3], [ben.account.address, 1.1], [org.account.address, 0.2]]) {
...
check((await bal(ana.address)) === 1, "ana chipped in $2 with one signature, relayer paid her fee");
...
check((await bal(ana.address)) === 0, "ana chipped $1 into pot B");
let refundErr = "";
